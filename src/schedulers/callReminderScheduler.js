...
export const fetchAllCallReminders = async () => {
  try {
    ...
    for (const reminder of callReminders) {
      ...
      // If never executed before, execute it
      if (!reminder.lastExecuted) {
        console.log(`Reminder ${reminder.title} has never been executed before`);
        shouldExecute = true;
      } else {
        ...
        switch (reminder.recurrence) {
          ...
          default:
            ...
            // Only execute if never executed before (handled above)
            break;
        }
      }
